...
def get_commute(home_lat, home_lon, work_address):
    ...
    if minutes is None:
        # Persist the geocoded coords so we don't re-geocode on next call
        if not cache.get("work_coords"):
            atomic_write_json(_CACHE_FILE, {**cache, "work_coords": work_coords})
        return cache.get("data")
