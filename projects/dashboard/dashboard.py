...
_financial_summary_lock = threading.Lock()
...
def _fetch_sources(source_names, ynab, budget_id, config):
    ...
    pool = ThreadPoolExecutor(max_workers=min(os.cpu_count() or 4, max(1, len(source_names))))
    futures = {}
...
def _build_today_panel(results, config):
    ...
    with _financial_summary_lock:
        fin = dict(_last_financial_summary)
...
def _build_header_badges(results, config=None):
    """Compute header info and badge counts for all tabs.

    Returns a JSON-serializable dict.
    """
    name = (config or {}).get("name", "Ian")
    now = datetime.now()
    hour = now.hour
    if hour < 12:
...
_last_badge_cache = {"badges": {}, "header": {}, "ts": 0.0}
_BADGE_CACHE_TTL = 300  # seconds; stale weather/next-event discarded after 5 minutes
...
def generate_html_dashboard(ynab, budget_id, config=None, default_tab=None):
    ...
    # ── Header data ──
    _name = config.get("name", "Ian")
    now = datetime.now()
    hour = now.hour
    if hour < 12:
        greeting = f"Good morning, {_name}"
    elif hour < 17:
        greeting = f"Good afternoon, {_name}"
    else:
        greeting = f"Good evening, {_name}"
...
<head>
    ...
    <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512'><defs><linearGradient id='bg' x1='0' y1='0' x2='1' y2='1'><stop offset='0%25' stop-color='%231a1a2e'/><stop offset='100%25' stop-color='%230d1b2a'/></linearGradient><linearGradient id='a' x1='0' y1='0' x2='0' y2='1'><stop offset='0%25' stop-color='%239f7aea'/><stop offset='100%25' stop-color='%23667eea'/></linearGradient><linearGradient id='b' x1='0' y1='0' x2='0' y2='1'><stop offset='0%25' stop-color='%23b794f4'/><stop offset='100%25' stop-color='%237f9cf5'/></linearGradient></defs><rect width='512' height='512' rx='96' fill='url(%23bg)'/><line x1='56' y1='340' x2='456' y2='340' stroke='%23667eea' stroke-width='1' opacity='.15'/><line x1='56' y1='260' x2='456' y2='260' stroke='%23667eea' stroke-width='1' opacity='.15'/><line x1='56' y1='180' x2='456' y2='180' stroke='%23667eea' stroke-width='1' opacity='.15'/><rect x='57' y='267' width='62' height='153' rx='10' fill='url(%23a)' opacity='.75'/><rect x='141' y='182' width='62' height='238' rx='10' fill='url(%23a)' opacity='.85'/><rect x='225' y='97' width='62' height='323' rx='10' fill='url(%23b)'/><rect x='309' y='216' width='62' height='204' rx='10' fill='url(%23a)' opacity='.85'/><rect x='393' y='148' width='62' height='272' rx='10' fill='url(%23a)' opacity='.75'/><rect x='56' y='420' width='400' height='3' rx='1.5' fill='%23667eea' opacity='.5'/></svg>">
    <title>Dashboard &mdash; {escape(budget['name'])}</title>
    <!-- PWA -->
...
def _start_server(port, ynab=None, budget_id=None, config=None):
    ...
    _refresh_queue: Queue = Queue(maxsize=1)  # at most one pending refresh; holds tab name or None
    ...
    def _do_refresh(ynab, budget_id, config, tab=None):
        ...
        try:
            ...
            try:
                queued_tab = _refresh_queue.get_nowait()
        ...
        def do_GET(self):
            ...
            if path == '/':
                ...
                if not _refresh_busy.is_set():
                    ...
                    # Replace any stale pending request then enqueue this one
                    try:
                        _refresh_queue.get_nowait()
                    except Empty:
                        pass
                    _refresh_queue.put_nowait(tab)
            ...
            if path == '/manage-feeds':
                try:
                    ...
                    for f in feeds:
                        ...
                        if not name or not url:
                            continue
                        if len(name) > 200 or len(url) > 500:
                            continue
                ...
                try:
                    ...
                    if not api_key:
                        ...
                        return
                    with _financial_summary_lock:
                        fin_snapshot = dict(_last_financial_summary)
                    result = query_claude_financial(
                        api_key, question, fin_snapshot
                    )
                    if result:
...
def _validate_config(config: dict) -> None:
    """Warn on common config mistakes. Does not exit — bad values fall back to defaults."""
    if "latitude" in config or "longitude" in config:
        lat = config.get("latitude")
        lon = config.get("longitude")
        if not isinstance(lat, (int, float)) or not (-90 <= lat <= 90):
            print(f"  Config warning: invalid latitude {lat!r} — using default")
            config.pop("latitude", None)
        if not isinstance(lon, (int, float)) or not (-180 <= lon <= 180):
            print(f"  Config warning: invalid longitude {lon!r} — using default")
            config.pop("longitude", None)

    if "news_feeds" in config:
        feeds = config["news_feeds"]
        if not isinstance(feeds, list):
            print(f"  Config warning: news_feeds must be a list — ignoring")
            config.pop("news_feeds")
        else:
            valid = []
            for f in feeds:
                if not isinstance(f, dict) or "name" not in f or "url" not in f:
                    print(f"  Config warning: skipping malformed feed entry {f!r}")
                    continue
                scheme = urlparse(str(f["url"])).scheme
                if scheme not in ("http", "https"):
                    print(f"  Config warning: skipping feed with non-http URL: {f['url']!r}")
                    continue
                valid.append(f)
            config["news_feeds"] = valid

    if "auto_refresh_minutes" in config:
        arm = config["auto_refresh_minutes"]
        if not isinstance(arm, (int, float)) or arm < 0:
            print(f"  Config warning: invalid auto_refresh_minutes {arm!r} — disabling")
