...
var SEED_APPS = [
    ...
    { name: 'Obsidian', url: "obsidian://open?vault=Ian's%20Obsidian%20folder", id: 'app_obsidian' },
];
...
function getApps() {
    ...
    // Merge: add any new seed apps not already stored
    var existingIds = {};
    apps.forEach(function(a) { existingIds[a.id] = true; });
    var added = false;
    SEED_APPS.forEach(function(seed) {
        if (!existingIds[seed.id]) {
            apps.push({ name: seed.name, url: seed.url, id: seed.id });
            added = true;
        }
    });
    if (added) saveApps(apps);
    return apps;
}
